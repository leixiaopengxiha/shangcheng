...
<div>
    <el-table ref="tableDD4" id="tableDD4" :data="tableData1" :span-method="objectSpanMethod" max-height="600" border>
      <template e v-for="(item, index) in arrlist" :key="index" >
        <el-table-column :label="item.label" :prop="item.key" :align="item?.align" :width="item?.width" />
      </template>
...
<script>
  export default {
    props:["arrlist","data","tableautomerge"],
    data() {
      return {
